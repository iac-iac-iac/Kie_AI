  ...
  if (generations.length === 0) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-3 p-6 text-center">
        ...
        <p className="text-sm text-muted">{t("video.emptyGallery")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 gap-2 overflow-y-auto p-3">
      {generations.map((gen) => (
        <div
          key={gen.id}
          className={`group relative aspect-video cursor-pointer overflow-hidden rounded-xl border transition-transform hover:scale-[1.01] ${
            selectedId === gen.id
