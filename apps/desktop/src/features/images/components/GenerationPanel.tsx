...
import { useModelSchema } from "../hooks/useImageModels";
...
export function GenerationPanel({
  ...
  useGenerationPoll(activeGenerationId, "image");
  ...
  const handleRetry = async () => {
    ...
    try {
      ...
      void queryClient.invalidateQueries({ queryKey: ["generations", "image"] });
    } catch (err) {
      setError(mapApiError(err, t));
    } finally {
      setRetrying(false);
    ...
    <div className="relative flex min-h-0 flex-1 flex-col">
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="space-y-3 border-t border-[var(--glass-border)] p-4">
          ...
          >
            {isGenerating ? t("images.generating") : t("images.generate")}
          </Button>
        </div>
      </div>
    </div>
