...
export function ImagesPage() {
  ...
  const { models, isLoading, isError, refetch } = useImageModels();
  ...
  useGenerationPoll(selectedIsActive ? selectedId : null, "image");
  ...
  if (isLoading) {
    return (
      <Card className="mx-auto max-w-2xl text-center">
        <p className="text-muted">{t("images.loading")}</p>
      </Card>
    );
  }
    ...
    <div className="flex h-full min-h-0 gap-4">
      <aside className="glass-panel flex w-[280px] shrink-0 flex-col rounded-2xl border border-[var(--glass-border)]">
        <div className="border-b border-[var(--glass-border)] px-4 py-3">
          <h2 className="text-sm font-medium text-primary">{t("images.gallery")}</h2>
        </div>
        <ImageGallery
